...
public interface GradeItemService {
    ...
    //GI-1) 평가항목을 생성하기 위한 service 선언부
    GradeItemResponseDTO createGradeItem(GradeItemCreateDTO dto);
    ...
    //GI-4) 기존에 존재하는 평가항목의 정보를 수정하기 위한 service 선언부
    GradeItemResponseDTO updateGradeItem(Long itemId, GradeItemUpdateDTO dto);

//    //GI-5) 평가항목을 삭제하기 위한 service 선언부
//    void deleteGradeItem(Long itemId);

    //GI-6) 평가항복의 비율의 합계를 검증하기 위한 service 선언부
    boolean validateWeightSum(Long offeringId);

    //GI-7) 평가항목의 개수를 조회하는 다른 serviceImpl 구현부에서 사용하기 위한 service 선언부
    Long countOfferingGradeItems(Long offeringId);
    
    //GI-8) Entity 조회를 하여 외부 다른 serviceImpl 구현부에서 사용하기 위한 service 선언부
